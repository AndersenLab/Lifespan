...
output:
  html_document:
    ...
    toc_float:
      ...
      smooth_scroll: false
---
This code uses processed survival data from Chris Fang-Yen's lab to calculate heritability of survival traits. The data were collected in 2 blocks with 3 WorMotels per block (independent bleaches) by Matt Churgin at UPenn. Each block was prepared using HTA strain handling protocol from ECA lab. In all cases 11-12 strains were run on a single wormMotel with (20 nematodes / strain).
<br>
...
### 2 blocks, 2 worMotels each
```{r, echo=FALSE, warning=FALSE}
# calculate heritability for ratio
n_range = c(3:20)
h2_table_good_blocks_two <- data.frame(trait=character(),
                       H2 = double(),
                       sample_size = numeric(), 
                       run = numeric()) 
h2_table_temp <- data.frame(trait=character(),
...
for (n in n_range){
  for (i in 1:10){
    ...
    df_subsample1 <- full_dat %>%
      ...
      #dplyr::select(-plate_num, -worm_num) %>%
      dplyr::group_by(replicate_num, strain) %>%
      sample_n(size = 2*n, replace = FALSE) %>%
      dplyr::mutate(new_plate_num = rep(seq(1,2, by = 1), n)) %>%
      dplyr::ungroup()
    ...
    #calculate means of all subsampled wells for a replicate bleach, regressed data by bleach
    df_subsample3 <- df_subsample2  %>%
      dplyr::group_by(replicate_num, new_plate_num, strain) %>%
      dplyr::mutate(mean_lifespan_days = mean(lifespan_days),
                    mean_int_b_activity = mean(w_int_b_activity),
                    mean_int_s_activity = mean(w_int_s_activity),
                    mean_lifespan_days_regressed = mean(lifespan_days_regressed),
                    ...
                    mean_int_s_activity_regressed = mean(w_int_s_activity_regressed)) %>%
      dplyr::group_by(replicate_num, strain) %>%
      dplyr::mutate(mean_lifespan_days = mean(mean_lifespan_days),
                    mean_int_b_activity = mean(mean_int_b_activity),
                    mean_int_s_activity = mean(mean_int_s_activity),
                    mean_lifespan_days_regressed = mean(mean_lifespan_days_regressed),
                    mean_int_b_activity_regressed = mean(mean_int_b_activity_regressed), 
                    mean_int_s_activity_regressed = mean(mean_int_s_activity_regressed)) %>%
      dplyr::ungroup()
    ...
    #join H2 table together 
    h2_table_good_blocks_two <- bind_rows(h2_table_temp, h2_traits_temp)
    h2_table_temp <- h2_table_good_blocks_two 
  }}

#Broad Sense Heritability, subsampling n worms from each strain in all 4 blocks

broad_sense_heritability_subsampling_good_blocks_two <- ggplot(h2_table_good_blocks_two) +
  geom_point(aes(x = sample_size, y = H2), alpha = 0.5, size = 1) +
  theme_light() +
  facet_wrap(~trait, nrow = 2) +
  ylim(0,1) +
  ...
  theme(title = element_text(size = 12, color = "black", face = "bold"),
        axis.title = element_text(size = 12, color = "black", face = "bold"),
        axis.text = element_text(size = 12, color = "black"))
broad_sense_heritability_subsampling_good_blocks_two
```

### 2 blocks, 3 worMotels each
```{r, echo=FALSE, warning=FALSE}
# calculate heritability for ratio
n_range = c(3:20)
...
```
<br>
Sampled worMotels from blocks 3 and 4 only. The sampled worMotels had 3-20 worms per strain. I calculated H2 using block means of worMotel means.
